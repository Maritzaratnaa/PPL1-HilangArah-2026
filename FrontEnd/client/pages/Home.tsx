...
const recommendedRoutes = [
  ...
  { icon: '🚈', name: 'Manggarai → Bogor', meta: 'KRL Commuterline · 80 mnt', accessible: false, duration: '80 mnt' },
];

const reports = [
  { icon: '🛗', name: 'Lift Stasiun Mati', location: 'MRT Bundaran HI', time: '2 jam lalu', status: 'Diproses', statusColor: 'yellow' },
  { icon: '🚧', name: 'Trotoar Rusak', location: 'Jl. Sudirman', time: '2 hari lalu', status: 'Selesai', statusColor: 'green' },
  { icon: '♿', name: 'Ramp Rusak', location: 'Halte Semanggi', time: '3 hari lalu', status: 'Menunggu', statusColor: 'yellow' },
];

const quickActions = [
  { icon: <MapPin className="h-6 w-6" />, label: 'Cari Rute', sub: 'Rute aksesibel', bg: 'bg-emerald-50 dark:bg-emerald-950/30', color: 'text-emerald-600', href: '/route-search' },
  { icon: <UserCheck className="h-6 w-6" />, label: 'Pesan Pemandu', sub: 'Tersedia sekarang', bg: 'bg-amber-50 dark:bg-amber-950/30', color: 'text-amber-600', href: '#' },
...
function Badge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${map[status]}`}>
      {status === 'green' ? '✅ Selesai' : '⏳ ' + (status === 'yellow' ? '' : '')}
    </span>
  );
}
          ...
          <div className="grid lg:grid-cols-[1fr_260px] gap-8 items-end">
            ...
            <div>
              <p className="text-white/60 text-sm font-semibold mb-1">
                Rabu, 11 Maret 2026 · 🌤 Jakarta, 28°C
              </p>
              <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">
                Halo, <span style={{ color: '#7dd8a6' }}>Budi</span> 👋
              </h1>
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                ...
                <span className="px-3 py-1 rounded-full text-xs font-semibold"
                  style={{ background: 'rgba(125,216,166,0.2)', color: '#7dd8a6', border: '1px solid rgba(125,216,166,0.3)' }}>
                  ⭐ Subscriber Aktif
                </span>
              </div>
              ...
              <div className="rounded-xl p-4 mb-0"
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 flex items-center gap-3 rounded-lg px-4 py-2.5"
                    ...
                    <span className="text-white/50 text-sm">📍 Lokasi saat ini</span>
                  </div>
                  ...
                  <Link to="/route-search">
                    <Button className="bg-white font-bold text-sm px-5 h-10 rounded-lg hover:-translate-y-0.5 transition-all"
            ...
            <div className="rounded-xl p-5 mb-0 self-end"
              style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)' }}>
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold mb-3"
                style={{ background: 'rgba(125,216,166,0.2)', color: '#7dd8a6', border: '1px solid rgba(125,216,166,0.3)' }}>
                ⭐ Aktif
              </div>
              <div className="text-white font-bold text-sm mb-1">Paket Bulanan</div>
              <div className="text-white/55 text-xs mb-3">Berlaku hingga 31 Mar 2026</div>
              ...
              <Link to="/subscription">
                <button className="w-full rounded-lg py-2 text-xs font-bold transition-all hover:opacity-90"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className="bg-background rounded-xl border border-border p-5
                ...
                <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">{s.label}</div>
            ...
            {quickActions.map((qa, i) => (
              <Link key={i} to={qa.href}>
                <div className="bg-background rounded-xl border border-border p-5
                  ...
                  <div className="text-sm font-bold text-foreground">{qa.label}</div>
                  <div className="text-xs text-muted-foreground mt-1">{qa.sub}</div>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <Link to="/route-search" className="text-sm font-bold text-primary flex items-center gap-1 hover:underline">
    ...
    high-contrast:border-2 high-contrast:border-primary">
                {recommendedRoutes.map((r, i) => (
                  <Link key={i} to="/route-search">
                    <div className="flex items-center gap-3 p-4 hover:bg-muted/40 transition-colors">
                      <div className="w-10 h-10 rounded-lg bg-emerald-50 dark:bg-emerald-950/30
            flex items-center justify-center text-lg flex-shrink-0">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${r.accessible
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <Link to="/reporting" className="text-sm font-bold text-primary flex items-center gap-1 hover:underline">
              ...
              <div className="bg-background rounded-xl border border-border divide-y divide-border
                ...
                {reports.map((r, i) => (
                  <div key={i} className="flex items-center gap-3 p-4 hover:bg-muted/40 transition-colors">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg flex-shrink-0 ${r.statusColor === 'green' ? 'bg-emerald-50 dark:bg-emerald-950/30' :
                        r.statusColor === 'yellow' ? 'bg-amber-50 dark:bg-amber-950/30' :
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold truncate">{r.name}</div>
                      <div className="text-xs text-muted-foreground">📍 {r.location} · {r.time}</div>
                    </div>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold flex-shrink-0 ${r.statusColor === 'green'
                        ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300'
                        : 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300'
                      }`}>
                      {r.statusColor === 'green' ? '✅ Selesai' : `⏳ ${r.status}`}
                    </span>
                  </div>
                ))}
              </div>
