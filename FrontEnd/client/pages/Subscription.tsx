...
export default function Subscription() {
  ...
  const benefits = [
    ...
    { icon: '🛡️', title: 'Keamanan Terjamin', description: 'Setiap pemandu melalui verifikasi identitas, pelatihan keselamatan, dan pemeriksaan latar belakang ketat.' },
    { icon: '🗺️', title: 'Rute Aksesibel Terencana', description: 'Pemandu membantu merencanakan rute terbaik yang ramah disabilitas sesuai tujuan perjalanan Anda.' },
    { icon: '📞', title: 'Kontak Darurat Terintegrasi', description: 'Sistem kontak darurat terintegrasi memastikan bantuan selalu tersedia kapanpun dibutuhkan.' },
    { icon: '⏰', title: 'Tersedia Sesuai Jadwal', description: 'Atur jadwal pertemuan dengan pemandu sesuai kebutuhan dan rencana perjalanan Anda.' },
    { icon: '💬', title: 'Dukungan 24/7', description: 'Tim support ARAHIN siap membantu Anda setiap saat melalui chat, telepon, maupun email.' },
  ];

  const faqItems = [
    { question: 'Apa itu program berlangganan ARAHIN?', answer: 'Program berlangganan ARAHIN memberikan Anda akses ke layanan pemandu pribadi yang siap menemani perjalanan menggunakan transportasi umum. Pemandu kami telah terlatih untuk mendampingi pengguna dengan berbagai kebutuhan khusus.' },
    ...
    { question: 'Apakah saya bisa membatalkan langganan?', answer: 'Ya, Anda dapat membatalkan langganan kapan saja tanpa biaya tambahan. Akses layanan akan tetap berlaku hingga akhir periode berlangganan yang sudah dibayar.' },
    ...
    <div className="min-h-screen flex flex-col bg-white font-sans">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#005260] via-[#007C8A] to-[#009DAD] pt-20 pb-28 px-6 lg:px-10">
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
          <div>
            <div className="inline-flex items-center gap-2 bg-white/15 border border-white/25 rounded-full px-4 py-1.5 text-xs text-white mb-6">
              <span className="text-yellow-400">✦</span> Program Unggulan ARAHIN
            </div>
            <h1 className="text-4xl lg:text-5xl font-medium text-white leading-[1.2] mb-5">
              Jalan Lebih Nyaman <br />
              <span className="opacity-85">dengan Pemandu Pribadi</span>
            </h1>
            <p className="text-[#ffffffb8] text-base leading-relaxed mb-9 max-w-md">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={i} className="bg-white/10 border border-white/15 rounded-xl p-4 text-center">
                  <div className="text-xl font-medium text-white">{stat.val}</div>
                  <div className="text-[10px] text-white/50 leading-tight mt-1">{stat.label}</div>
                </div>
              ))}
            </div>
            <div className="bg-white/12 border border-white/20 rounded-2xl p-5 flex items-center gap-4">
              <div className="w-11 h-11 rounded-full bg-white/20 flex items-center justify-center text-white font-medium text-sm">RS</div>
              <div>
                <div className="text-sm font-medium text-white">Rizky Santoso</div>
                <div className="text-[11px] text-white/50">Pemandu KRL Jabodetabek · ⭐ 4.9</div>
              </div>
              <div className="ml-auto bg-green-500/20 border border-green-500/40 rounded-full px-3 py-1 text-[10px] text-green-300 flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" /> Aktif
              </div>
            </div>
            <p className="text-[11px] text-white/40 text-center mt-1">Cocokkan pemandu sesuai kebutuhan Anda</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#F8FAFB] py-20 px-6 lg:px-10">
        <div className="max-w-5xl mx-auto">
          <div className="mb-12">
            <span className="text-[11px] font-semibold text-[#007C8A] uppercase tracking-widest block mb-2">Keunggulan Program</span>
            <h2 className="text-3xl font-medium text-gray-900 mb-3">Mengapa Berlangganan ARAHIN?</h2>
            <p className="text-gray-500 text-base max-w-md">Setiap fitur dirancang khusus untuk kenyamanan dan keamanan perjalanan Anda</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {benefits.map((b, i) => (
              <div key={i} className="bg-white border border-gray-100 p-8 rounded-2xl hover:border-[#007C8A] transition-colors group cursor-default">
                ...
                <h3 className="text-[15px] font-medium text-gray-900 mb-2">{b.title}</h3>
      ...
      <section id="pricing" className="bg-[#007C8A] py-24 px-6 lg:px-10 relative overflow-hidden text-white">
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-6">
            <span className="text-xs font-medium text-white/60 uppercase tracking-widest">Investasi Terbaik</span>
            <h2 className="text-3xl font-medium leading-tight">Satu Langkah Menuju Perjalanan yang Lebih Mudah</h2>
            <p className="text-white/70 max-w-sm">Bergabung dengan ribuan pengguna yang sudah merasakan manfaat pemandu pribadi ARAHIN setiap harinya.</p>
          ...
          <Card className="bg-white border-none rounded-[20px] p-9 shadow-xl text-gray-900 relative z-10">
            <div className="inline-flex items-center gap-1.5 bg-[#E6F4F6] text-[#007C8A] px-3 py-1 rounded-full text-xs font-semibold mb-5">
              <Star size={12} fill="currentColor" /> Paling Populer
            </div>
            <div className="text-[13px] text-gray-500 mb-2">Paket Bulanan</div>
            <div className="text-4xl font-medium text-[#007C8A] flex items-baseline gap-1">
              Rp 299.000<span className="text-base text-gray-400 font-normal">/bln</span>
            </div>
            <div className="text-[12px] text-gray-400 mt-2 mb-6">Tagihan bulanan · batalkan kapan saja</div>
      ...
      <section className="bg-[#F8FAFB] py-20 px-6">
        <div className="max-w-2xl mx-auto">
          <div className="mb-10">
            <span className="text-[11px] font-semibold text-[#007C8A] uppercase tracking-widest block mb-2">FAQ</span>
            <h2 className="text-3xl font-medium text-gray-900">Pertanyaan yang Sering Ditanyakan</h2>
          </div>
              ...
              <div key={idx} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                ...
                >
                  ...
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center border transition-all ${
                    expandedFaq === idx ? 'bg-[#E6F4F6] border-[#E6F4F6] text-[#007C8A] rotate-45' : 'bg-gray-50 border-gray-200 text-gray-400'
                  }`}>
                    <span className="text-lg leading-none">+</span>
                  </div>
                </button>
                <div className={`transition-all duration-300 ease-in-out overflow-hidden ${
                  expandedFaq === idx ? 'max-h-40 border-t border-gray-100' : 'max-h-0'
                }`}>
                  <div className="p-5 text-[13px] text-gray-500 leading-relaxed">
                    {item.answer}
                  </div>
                </div>
              </div>
          ...
          <div className="text-center mt-12">
            <p className="text-[14px] text-gray-500 mb-3">Masih ada pertanyaan?</p>
            <button className="border border-[#007C8A] text-[#007C8A] px-6 py-2.5 rounded-lg text-[14px] font-medium hover:bg-[#007C8A] hover:text-white transition-all">
              Hubungi Tim Kami
            </button>
          </div>
        </div>
