...
import {
  BarChart3, Users, UserCheck, Zap, FileText, Bus, LogOut, ChevronUp, KeyRound, X, Loader2, Eye, EyeOff
} from 'lucide-react';
...
export function AdminSidebar() {
  ...
  // Decode JWT untuk ambil email admin yang sedang login
  ...
  return (
    <>
      <aside className="w-64 flex-shrink-0 border-r border-border bg-muted/30 flex flex-col fixed top-0 left-0 h-screen z-40">
        ...
        <div className="p-4 border-t border-border flex-shrink-0 relative">
          {isAccountOpen && (
            <div className="absolute bottom-20 left-4 right-4 bg-card border border-border rounded-xl shadow-lg p-2 animate-in fade-in slide-in-from-bottom-2">
              <button
                onClick={() => { setIsAccountOpen(false); setShowEditPassword(true); }}
                className="w-full flex items-center gap-3 px-3 py-2 text-sm font-semibold text-foreground hover:bg-muted rounded-lg transition-colors">
                <KeyRound className="h-4 w-4" />
                Ubah Password
              </button>
              <button
                onClick={handleLogout}
                className="w-full flex items-center gap-3 px-3 py-2 text-sm font-semibold text-rose-600 hover:bg-rose-50 rounded-lg transition-colors">
